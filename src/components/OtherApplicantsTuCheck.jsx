...
import { 
  ...
  setFilters 
} from '../redux/slices/otherApplicantsTuSlice';
import '../styles/other-applicants-tu.scss';

const OtherApplicantsTuCheck = () => {
  const dispatch = useDispatch();
  const { otherApplicantsTus, filters } = useSelector(state => state.otherApplicantsTu);
  ...
  const getApplicantTypeColor = (type) => {
    switch (type) {
      case 'ИП': return '#9C27B0';
      case 'ООО': return '#2196F3';
      case 'ОАО': return '#4CAF50';
      case 'ГУП': return '#FF9800';
      default: return '#757575';
    }
  };

  const filteredTus = otherApplicantsTus.filter(tu => {
      ...
      <div className="tu-header">
        <div className="header-content">
          <h1>
            ...
            ТУ других заявителей
          </h1>
          <p>Проверка технических условий для различных типов заявителей</p>
        </div>
        <div className="header-stats">
          <div className="stat-item">
            <span className="stat-number">{otherApplicantsTus.length}</span>
            <span className="stat-label">Всего ТУ</span>
          </div>
          <div className="stat-item">
            <span className="stat-number">
              {otherApplicantsTus.filter(tu => tu.status === 'Завершена').length}
            </span>
            <span className="stat-label">Завершено</span>
          </div>
          <div className="stat-item">
            <span className="stat-number">
              {otherApplicantsTus.filter(tu => tu.status === 'В процессе проверки').length}
            </span>
          ...
          >
            <div className="card-header">
              <div className="card-title">
                ...
                <div className="badges">
                  ...
                  <span 
                    className="applicant-type-badge"
                    style={{ backgroundColor: getApplicantTypeColor(tu.applicantType) }}
                  >
                    {tu.applicantType}
                  </span>
                </div>
            ...
            <div className="card-content">
              ...
              <div className="info-row">
                ...
                <span className="value">{tu.voltage}</span>
              </div>
              <div className="info-row">
